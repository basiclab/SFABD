...
class ProposalConv(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.conv1x1_iou(x)
        
        ## use separate embedding space
        #return out1, out2

        ## only use iou embedding space
        return out


def build_proposal_conv(cfg, mask2d):
    input_size = cfg.MODEL.MMN.FEATPOOL.HIDDEN_SIZE ## 512
    hidden_size = cfg.MODEL.MMN.PREDICTOR.HIDDEN_SIZE ## 512
    kernel_size = cfg.MODEL.MMN.PREDICTOR.KERNEL_SIZE ## 5
    num_stack_layers = cfg.MODEL.MMN.PREDICTOR.NUM_STACK_LAYERS ## 8
    output_size = cfg.MODEL.MMN.JOINT_SPACE_SIZE ## 256

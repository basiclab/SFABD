...
def build_dataset(dataset_list, dataset_catalog, cfg, is_train=True):
    ...
    datasets = []
    for dataset_name in dataset_list: ## 'charades_combined_test'
        """
        dataset_catalog.get method
        ...
        factory = getattr(D, data["factory"]) ## factory = datasetname
        args = data["args"]
        args["num_pre_clips"] = cfg.INPUT.NUM_PRE_CLIPS
        args["num_clips"] = cfg.MODEL.MMN.NUM_CLIPS
        args["feat_type"] = cfg.MODEL.MMN.FEAT_TYPE
        dataset = factory(**args) ## initiate dataset instance
...
def make_data_loader(cfg, is_train=True, is_distributed=False, is_for_period=False):
    ...
    if is_train:
        ...
        assert (
            ...
            batch_size, num_gpus)
        batch_size_per_gpu = batch_size // num_gpus
        ## True when testing (is_distributed is false)
    ...
    dataset_list = cfg.DATASETS.TRAIN if is_train else cfg.DATASETS.TEST ## 'charades_combined_test'
    #dataset_list = cfg.DATASETS.TRAIN ## !! Modified

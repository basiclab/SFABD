...
from src.utils import scores2ds_to_moments
...
class MMN_bbox_reg(nn.Module):
    ...
    def forward(
        ...
        **kwargs,                           # dummy
        ...
        start_offset, end_offset = self.bbox_head(
                                        video_feats1[scatter_s2v], 
                                        sents_feats1
                                    )                                       # [S, 2, N, N] 
        
        S = start_offset.shape[0]
        start_offset = start_offset.masked_select(mask2d).view(S, -1)       # [S, P]
        end_offset = end_offset.masked_select(mask2d).view(S, -1)           # [S, P]
        anchor, scores1ds = scores2ds_to_moments(scores2d.detach(), mask2d.detach())
        bbox = anchor + torch.stack((start_offset, end_offset), dim=-1)     # [S, P, 2]
